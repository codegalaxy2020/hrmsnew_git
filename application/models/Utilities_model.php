...
class Utilities_model extends App_Model
{
    ...
    public function get_all_training($start, $end)
    {
        // ini_set('display_errors', 1);
        // ini_set('display_startup_errors', 1);
        // error_reporting(E_ALL);
        $is_staff_member = is_staff_member();
        // $this->db->select('training_name,time_to_start,time_to_end,training_process_id,staff_id');
        $this->db->select('training_name as title, time_to_start as start, time_to_end as end, training_process_id as eventid, staff_id as userid');
        // Check if is passed start and end date
        $this->db->where('(time_to_start BETWEEN "' . $start . '" AND "' . $end . '")');
        // $this->db->where('userid', get_staff_user_id());
        // if ($is_staff_member) {
        //     $this->db->or_where('public', 1);
        // }
    ...
    public function get_event($id)
    {
        ...
        public function get_event_training($id)
    {
    ...
    public function get_calendar_data_training($start, $end, $client_id = '', $contact_id = '', $filters = false)
    {
        ...
        $events = $this->get_all_training($start, $end);
        // print_r($events);
            foreach ($events as $event) {
                if ($event['userid'] != get_staff_user_id() && !$is_admin) {
                    $event['is_not_creator'] = true;
                ...
                $event['url']      = 'https://skilltest.live/hrmsnew/admin/hr_profile/view_training_program/' . $event['eventid'];
                $event['_tooltip'] = _l('calendar_event') . ' - ' . $event['title'];
                $event['color']    = $event['color'];
                array_push($data, $event);
            }
