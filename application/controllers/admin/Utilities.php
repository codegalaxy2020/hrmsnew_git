...
class Utilities extends AdminController
{
    ...
    /* Calendar functions */
    public function calendartraining()
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            $data = $this->input->post();
        ...
        $this->load->view('admin/utilities/calendartraining', $data);
    }
    ...
    public function get_calendar_data_training()
    {
        echo json_encode($this->utilities_model->get_calendar_data_training(
                date('Y-m-d', strtotime($this->input->get('start'))),
                date('Y-m-d', strtotime($this->input->get('end'))),
                '',
                '',
    ...
    public function view_event_training($id)
    {
        
        $data['event'] = $this->utilities_model->get_event_training($id);
        // if ($data['event']->public == 1 && !is_staff_member()
        //     || $data['event']->public == 0 && $data['event']->userid != get_staff_user_id()) {
        // } else {
            // print_r($data['event']);
