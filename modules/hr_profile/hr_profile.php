...
/*
hooks()->add_action('app_init',HR_PROFILE_MODULE_NAME.'_appint');
function hr_profile_appint(){
    $CI = & get_instance();
    $CI->load->library(HR_PROFILE_MODULE_NAME.'/Envhelpers');
    $envato_res = $CI->envhelpers->validate_purchase(HR_PROFILE_MODULE_NAME);
    if (!$envato_res) {
...
function hr_profile_preactivate($module_name){
    if ($module_name['system_name'] == HR_PROFILE_MODULE_NAME) {
        if (!option_exists(HR_PROFILE_MODULE_NAME.'_verified') && empty(get_option(HR_PROFILE_MODULE_NAME.'_verified')) && !option_exists(HR_PROFILE_MODULE_NAME.'_verification_id') && empty(get_option(HR_PROFILE_MODULE_NAME.'_verification_id'))) {
            $CI = & get_instance();
...
function hr_profile_predeactivate($module_name){
    if ($module_name['system_name'] == HR_PROFILE_MODULE_NAME) {
        delete_option(HR_PROFILE_MODULE_NAME."_verified");
        delete_option(HR_PROFILE_MODULE_NAME."_verification_id");
        delete_option(HR_PROFILE_MODULE_NAME."_last_verification");
