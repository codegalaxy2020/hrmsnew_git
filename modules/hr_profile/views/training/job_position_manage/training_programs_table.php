...
//load deparment by manager
 if(!is_admin() && !has_permission('staffmanage_training','','view')){
 	...
 	if (count($array_staff) == 0) {
        ...
        // Add the condition where staff_ is equal to get_staff_user_id()
        $staffUserId = get_staff_user_id();
        $where[] = "AND FIND_IN_SET($staffUserId, staff_id) > 0";
    }
 }

$result       = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['job_position_id', 'position_training_id', 'additional_training', 'training_venues', 'training_location', 'staff_id', 'time_to_start', 'time_to_end', 'status','direct_cost','indirect_cost']);
// print_r($result);die();
	...
	$row = [];
		...
		$row[] = $aRow['training_process_id'];
			...
			if (has_permission('staffmanage_training', '', 'edit')) {
				$subject .= ' | <a href="#" onclick="edit_training_process(this,' . $aRow['training_process_id'] . ', '.$aRow['training_process_id'].');return false;" data-id_training= "'.$aRow['training_process_id'].'" data-training_name= "'.$aRow['training_name'].'" data-training_location= "'.$aRow['training_location'].'" data-training_venues= "'.$aRow['training_venues'].'" data-indirect_cost= "'.$aRow['indirect_cost'].'" data-direct_cost= "'.$aRow['direct_cost'].'"  data-job_position_training_type= "'.$aRow['training_type'].'" data-job_position_mint_point= "'.$aRow['mint_point'].'"  data-job_position_training_id= "'.$aRow['position_training_id'].'" data-job_position_id= "'.$aRow['job_position_id'].'" data-additional_training= "'.$aRow['additional_training'].'" data-staff_id= "'.$aRow['staff_id'].'" data-time_to_start= "'._d($aRow['time_to_start']).'" data-time_to_end= "'._d($aRow['time_to_end']).'" >' . _l('hr_edit') . '</a>';
			}    
			...
			if (has_permission('staffmanage_training', '', 'delete')) {
			    ...
			    if (has_permission('staffmanage_training', '', 'view') || has_permission('staffmanage_training', '', 'view_own')) {
			        if(is_admin()){
			        if($aRow['status'] == "0"){
			            $subject .= ' | <a href="' . admin_url('hr_profile/cancel_job_position_training_process/' . $aRow['training_process_id']) . '" style="color: #a00000;" class="text-danger _delete">Cancel</a>';
			        } else {
			            
			            $subject .= ' | <a href="' . admin_url('hr_profile/active_job_position_training_process/' . $aRow['training_process_id']) . '" style="color: #a00000;" class="text-danger _delete">Active</a>';   
			        }
			        } else {
			            if($aRow['status'] == "0"){
			            $subject .= ' | <a href="#" style="color: #a00000;" class="text-danger">Active</a>';
			        } else {
			            
			            $subject .= ' | <a href="#" style="color: #a00000;" class="text-danger">Canceled</a>';   
			        }
			        }
			    }
