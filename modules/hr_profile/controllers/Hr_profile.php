...
class Hr_profile extends AdminController {
	...
	public function training() {
		...
		if (!is_admin()) {
			$data['tab'][] = 'others_training_program';
		}
		if (is_admin()) {
			$data['tab'][] = 'waitlist';
		}
		if (is_admin()) {
			$data['tab'][] = 'training_attendance';
		}
...
// 		print_r($data['faculty_data']);die();
	...
	public function deleteFaculty() {
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            $facultyId = $this->input->post('id');
            $result = $this->hr_profile_model->deleteFaculty($facultyId);

            if ($result) {
                echo json_encode(['status' => 'success', 'message' => 'Faculty deleted successfully.']);
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Error deleting faculty.']);
            }
        } else {
...
// 		print_r($data['type_of_trainings']);die();
		...
		$this->load->view('hr_profile/training/job_position_manage/faculty', $data);
    }
...
// 			$data['description'] = $this->input->post('description', false);
			...
			if ($id == '') {
				...
				$id = $this->hr_profile_model->add_faculty($data);
				if ($id) {
					set_alert('success', _l('added_successfully', _l('hr_training')));
					redirect(admin_url('hr_profile/manage_faculty'));
				}
			} else {
				
				$success = $this->hr_profile_model->add_faculty($data, $id);
				if ($success) {
					set_alert('success', _l('updated_successfully', _l('hr_training')));
				}
				redirect(admin_url('hr_profile/manage_faculty'));
		...
		if ($id == '') {
			$title = _l('add_new', _l('faculty'));
		} else {
		    $faculty_data_id = $this->hr_profile_model->faculty_data_id($id);
		  //  $data['type_of_trainings'] = $this->hr_profile_model->get_training_program();
		  //  print_r($faculty_data_id[0]['faculty']);die();
// 			$position_training = $this->hr_profile_model->get_training_program($faculty_data_id[0]['training_program']);
// 			$data['position_training'] = $position_training;
			$data['faculty_data_id'] = $faculty_data_id[0];
			$title = $faculty_data_id[0]['faculty'];
		}
...
// 		print_r($data['type_of_trainings']);die();
	 ...
	 */
	public function check_allotted_slots(){
	    $training_id = $this->input->post('training_id');
	    $allotted_slots = $this->hr_profile_model->get_allotted_slots($training_id);
	    if ($allotted_slots > 0) {
        $staffUserId = get_staff_user_id();
        $this->hr_profile_model->update_allotted_slots($training_id, $staffUserId);
        // print_r($allotted_slots);die();
        echo json_encode(array('success' => true, 'message' => 'You can join now!'));
    } else {
        $staffUserId = get_staff_user_id();
        $this->hr_profile_model->update_waitlist_slots($training_id, $staffUserId);
        echo json_encode(array('success' => false, 'message' => 'No available slots.'));
    }
	}
	public function attendance()
    {
        ...
        // Check if the record already exists for the given staff, lead, and date
	...
	public function check_allotted_slots_join(){
	    $training_id = $this->input->post('training_id');
	    $staffId = $this->input->post('staffId');
	   ...
	   // if ($allotted_slots > 0) {
        $staffUserId = get_staff_user_id();
        $this->hr_profile_model->update_allotted_slots_join($training_id, $staffId, $training_name);
        // print_r($allotted_slots);die();
        echo json_encode(array('success' => true, 'message' => 'You can join now!'));
	...
	public function view_attendance($wait_staff_id, $training_process_id) {
		...
		$this->load->view('hr_profile/training/view_attendance', $data);
	}
	
public function attendance_data_id() {
            $staffId = $this->input->post('staffId');
            $leadId = $this->input->post('leadId');
            // echo $staffId;
            $attendanceData = $this->hr_profile_model->getAttendanceData($staffId, $leadId);
	...
	public function wait_staff_id_cancel(){
	    $training_id = $this->input->post('training_id');
	    $staffId = $this->input->post('staffId');
	    $training_name = $this->input->post('training_name');
        $staffUserId = get_staff_user_id();
        $this->hr_profile_model->update_wait_staff_id_cancel($training_id, $staffId, $training_name);
        echo json_encode(array('success' => true, 'message' => 'Staff Remove from Waitlist!'));
	...
	public function job_position_training_add_edit() {
		...
		if ($this->input->post()) {
			...
			if (!$this->input->post('id_training')) {
				...
				//send notification
                $notified = add_notification([
                    'description' => "added new training " . $data['training_name'],
                    'touserid' => $data['staff_id'][0],
                    'additional_data' => serialize([
                        'additional_description' => $data['description'],
                    ]),
                ]);
    			if ($notified) {
    				pusher_trigger_notification([$data['staff_id'][0]]);
    			}
				...
				//send notification
    			$notified = add_notification([
    				// 'description' => "Some Edit Done on your training " . $data['training_name'],
    				'description' => "Some Edit Done on your training " . $data['training_name'],
    				'touserid' => $data['staff_id'][0],
    				'additional_data' => serialize([
    					$data['description'],
...
public function get_list_job_position_training($id) {
		...
		if (isset($list)) {
			$description = $list->description;
			$special_needs = $list->special_needs;
			$expectation_from_training = $list->expectation_from_training;
		} else {
			$description = '';
			$special_needs = '';
			$expectation_from_training = '';

		}
		echo json_encode([
			'description' => $description,
			'special_needs' => $special_needs,
			'expectation_from_training' => $expectation_from_training,
	...
	public function cancel_job_position_training_process($training_id) {
		if (!has_permission('staffmanage_job_position', '', 'delete')) {
			access_denied('job_position');
		}
		...
		$success = $this->hr_profile_model->cancel_job_position_training_process($training_id);
		if ($success) {
			set_alert('success', 'Status Changed');
		} else {
			set_alert('warning', _l('problem_deleting'));
		}
		redirect(admin_url('hr_profile/training/?group=training_program'));
	...
	public function active_job_position_training_process($training_id) {
		if (!has_permission('staffmanage_job_position', '', 'delete')) {
			access_denied('job_position');
		}
		...
		$success = $this->hr_profile_model->active_job_position_training_process($training_id);
		if ($success) {
			set_alert('success', 'Status Changed');
		} else {
			set_alert('warning', _l('problem_deleting'));
...
// multiple deparment
	...
	public function get_training_program_by_type() {
		if ($this->input->is_ajax_request()) {
            ...
            // print_r($training_program_option);die();
			echo json_encode([
    ...
    public function cost_tracking() {

// 		$data['title'] = $title;
		$data['faculty_data'] = $this->hr_profile_model->cost_data1();
// 		print_r($data['faculty_data']);die();
		$this->app_scripts->add('surveys-js', module_dir_url('surveys', 'assets/js/surveys.js'), 'admin', ['app-js']);
		$this->app_css->add('surveys-css', module_dir_url('hr_profile', 'assets/css/training/training_post.css'), 'admin', ['app-css']);

		$this->load->view('hr_profile/training/cost_tracking', $data);
	}
...
// 		$data['title'] = $title;
		$data['faculty_data'] = $this->hr_profile_model->get_components();
// 		print_r($data['faculty_data']);die();
		$this->app_scripts->add('surveys-js', module_dir_url('surveys', 'assets/js/surveys.js'), 'admin', ['app-js']);
		$this->app_css->add('surveys-css', module_dir_url('hr_profile', 'assets/css/training/training_post.css'), 'admin', ['app-css']);

		$this->load->view('hr_profile/training/add_components', $data);
	}
	
	public function allocate_cost_tracking() {

// 		$data['title'] = $title;
        $data['allocated'] = $this->hr_profile_model->allocated(get_staff_user_id());
        $allocated_costids = array_column($data['allocated'], 'costid');
		$data['faculty_data'] = $this->hr_profile_model->cost_data($allocated_costids);
		
// 		print_r($data['faculty_data']);die();
		$this->app_scripts->add('surveys-js', module_dir_url('surveys', 'assets/js/surveys.js'), 'admin', ['app-js']);
		$this->app_css->add('surveys-css', module_dir_url('hr_profile', 'assets/css/training/training_post.css'), 'admin', ['app-css']);

		$this->load->view('hr_profile/training/allocated_cost_tracking', $data);
	}
	
	public function allocate_cost() {
        ...
        $this->db->where('userId', $userId)
                 ->where('costid', $rowId)
                 ->update('tblallocatecost', $updateData);

        // Return a response indicating successful update
        $response = array('status' => 'success', 'message' => 'Cost updated successfully');
        } else {
            // Entry does not exist, insert data into the tblallocatecost table
            $data = array(
                'userId' => $userId,
                'costid' => $rowId,
                'costType' => $costType
            ...
            $this->db->insert('tblallocatecost', $data);

            // Return a response indicating successful allocation
            $response = array('status' => 'success', 'message' => 'Cost allocated successfully');
        }
	...
	public function add_cost_tracking($id = '') {
		if ($this->input->post()) {
			$data = $this->input->post();
			...
			if ($id == '') {
				...
				$id = $this->hr_profile_model->add_cost($data);
				// print_r($id);die();
				if ($id) {
					set_alert('success','Cost Add Succesfully');
					redirect(admin_url('hr_profile/cost_tracking'));
				}
			} else {
				
				$success = $this->hr_profile_model->add_cost($data, $id);
				if ($success) {
					set_alert('success','Cost Update Succesfully');
				}
				redirect(admin_url('hr_profile/cost_tracking'));
			}
		}
		if ($id == '') {
		    ...
		    $faculty_data_id = $this->hr_profile_model->get_cost($id);
			$data['get_cost'] = $faculty_data_id;
// 			print_r($data['get_cost']);die();
			$title = 'Edit Cost';
		}
		...
		$this->load->view('hr_profile/training/job_position_manage/cost', $data);
	}
	
	// public function view_cost_tracking($id = '', $cost) {
	//    // print_r($cost);die();
	//     $faculty_data_id = $this->hr_profile_model->get_cost($id);
	// 	$data['get_cost'] = $faculty_data_id;
	// 	$data['cost'] =$cost;
	...
	// 	$this->app_scripts->add('surveys-js', module_dir_url('surveys', 'assets/js/surveys.js'), 'admin', ['app-js']);
	// 	$this->app_css->add('surveys-css', module_dir_url('hr_profile', 'assets/css/training/training_post.css'), 'admin', ['app-css']);
	// 	$this->load->view('hr_profile/training/job_position_manage/viewcost', $data);
	// }
	
// 	function generate_slug($string) {
//     $slug = strtolower(str_replace(' ', '_', $string)); // Replace spaces with underscores
//     $slug = preg_replace('/[^a-z0-9_]/', '', $slug); // Remove special characters
//     return $slug;
//     }
	
	public function addelementscost()
    {
        // Retrieve data from the Ajax request
        $costType = $this->input->post('costType'); // Assuming you are using CodeIgniter
        $name = $this->input->post('name');
        // Add other fields as needed

        // Validate input if necessary

        // Insert into tblcomponents table
        // $this->load->model('Tblcomponents_model'); // Assuming you have a model for tblcomponents
        $slug = strtolower(str_replace(' ', '_', $name)); // Replace spaces with underscores
        $slug = preg_replace('/[^a-z0-9_]/', '', $slug);
        $data = array(
            'costtype' => $costType,
            'name' => $name,
            'slug' => $slug,
            'status' => 'pending', // You might want to set a default status
            // Add other fields as needed
        );
        $this->hr_profile_model->addelementscost($data); // Adjust based on your model method
   ...
   public function deletecost() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            // Get the faculty ID from the POST data
            $facultyId = $this->input->post('id');

            // Call the method in the model to delete the faculty record
            $result = $this->hr_profile_model->deletecost($facultyId);

            // Check the result and send a response
            if ($result) {
                echo "Faculty deleted successfully";
            } else {
                echo "Error deleting faculty. Please try again.";
            }
        } else {
            // Invalid request
