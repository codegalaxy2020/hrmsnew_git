...
public function add_cost($data, $id = null) {
    ...
    $dynamicColumns = array_keys($data);

    foreach ($dynamicColumns as $column) {
        $dbData[$column] = $data[$column];
    }

    // Check and create dynamic columns in the database
    foreach ($dynamicColumns as $column) {
        if (!$this->db->field_exists($column, 'direct_cost_training')) {
            $this->dbforge->add_column('direct_cost_training', [
                $column => ['type' => 'DECIMAL(10,2)', 'default' => 0],
            ]);

            // Reload the database connection after adding a new column
    ...
    if ($id === null) {
        // Insert new record for adding
        $this->db->insert('tbldirect_cost_training', $dbData);
        $direct_cost_id = $this->db->insert_id();
        // print_r($direct_cost_id);die();
        if ($direct_cost_id) {
            log_activity('New Cost Added [ID: ' . $direct_cost_id . ']');
        ...
        $this->db->where('id', $id);
        $this->db->update('tbldirect_cost_training', $dbData);

        if ($this->db->affected_rows() > 0) {
            log_activity('Cost Updated [ID: ' . $id . ']');
            return $id;
        } else {
            // Return false or handle the failure as needed
            return false;
...
return false;
	...
	public function deletecost($facultyId) {
        // Perform the delete operation in the database
        $this->db->where('id', $facultyId);
        $result = $this->db->delete('tbldirect_cost_training');
