...
class Hr_profile_model extends App_Model
{
	...
	*/
	...
	public function getAttendanceData($staffId, $leadId) {
	     $this->db->select('*');
	     $this->db->where('staff_id', $staffId);
	     $this->db->where('training_id', $leadId);
         $query = $this->db->get(db_prefix() . 'training_attendance');
...
public function add_faculty($data, $id = null) {
    $datecreated = date('Y-m-d H:i:s');

    if ($id === null) {
        // Insert new record for adding
        $this->db->insert(db_prefix().'faculty', [
            'faculty'         => $data['subject'],
        ...
        log_activity('New Faculty Added [ID: ' . $trainingid . ', Subject: ' . $data['subject'] . ']');

        return $trainingid;
    } else {
        // Update existing record for editing
        $this->db->where('id', $id)->update(db_prefix().'faculty', [
            'faculty'         => $data['subject'],
            'training_program'   => $data['training_program'],
            'slug'            => slug_it($data['subject']),
            'description'     => $data['viewdescription'],
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('Faculty Updated [ID: ' . $id . ', Subject: ' . $data['subject'] . ']');
            return $id;
        } else {
            // Return false or handle the failure as needed
            return false;
...
public function add_cost($data, $id = null) {
    $datecreated = date('Y-m-d H:i:s');

    if ($id === null) {
        // Insert new record for adding
        $this->db->insert(db_prefix().'direct_cost_training', [
            'training_program_id' => $data['training_program'],
            'salary' => $data['salary'],
            ...
            'retirement_plans' => $data['retirement_plans'],
            'total' => $data['salary'] + $data['bonuses'] + $data['commissions'] + $data['allowances'] + $data['benefits'] + $data['healthcare'] + $data['retirement_plans'],
            'office_space' => $data['office_space'],
            'utilities' => $data['utilities'],
            'equipment' => $data['equipment'],
            'supplies' => $data['supplies'],
            'other_resources' => $data['other_resources'],
            'indirect_cost_total' => $data['office_space'] + $data['utilities'] + $data['equipment'] + $data['supplies'] + $data['other_resources'],
            'recruitment_cost' => $data['recruitment_cost'],
            'onboarding_cost' => $data['onboarding_cost'],
            'payroll_processing_cost' => $data['payroll_processing_cost'],
            'hr_personnel_cost' => $data['hr_personnel_cost'],
            'administrative_costs' => $data['recruitment_cost'] + $data['onboarding_cost'] + $data['payroll_processing_cost'] + $data['hr_personnel_cost'],
            'employee_training' => $data['employee_training'],
            'workshops' => $data['workshops'],
            'courses' => $data['courses'],
            'certifications' => $data['certifications'],
            'materials' => $data['materials'],
            'training_development_expenses_total' => $data['employee_training'] + $data['workshops'] + $data['courses'] + $data['certifications'] + $data['materials'],
        ]);

        $direct_cost_id = $this->db->insert_id();

        if (!$direct_cost_id) {
            // Handle the failure to insert into direct_cost_training table
            // You might want to consider rolling back the faculty insertion
            return false;
        }

        log_activity('New Cost Added [ID: ' . $trainingid . ', Subject: ' . $data['subject'] . ']');

        return $direct_cost_id;
    } else {
        $this->db->where('training_program_id', $data['training_program'])->update(db_prefix().'direct_cost_training', [
            'salary' => $data['salary'],
            'bonuses' => $data['bonuses'],
            'commissions' => $data['commissions'],
            'allowances' => $data['allowances'],
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('Cost Updated [ID: ' . $id . ', Subject: ' . $data['subject'] . ']');
            return $id;
        } else {
            // Return false or handle the failure as needed
            return false;
	...
	public function deleteFaculty($facultyId) {
        // Your database query to delete the faculty record
        $this->db->where('id', $facultyId);
        $this->db->delete('tblfaculty'); // Replace with your actual table name
...
// 		}
		...
		if ($insert_id) {
			if(isset($data['additional_training'])){
				if(isset($data_staff_id) && count($data_staff_id) > 0){
				    ...
				    //update slots
				    $this->db->select('slots, pending_slots, allotted_slots');
                    $this->db->where('id', $insert_id);
                    $query = $this->db->get('hr_jp_interview_training');
        
                    if ($query->num_rows() > 0) {
                        $row = $query->row_array();
        
                        // Update values based on your logic
                        $updated_slots = $row['slots'] - count($data_staff_id);
                        $updated_pending_slots = $row['pending_slots'] - count($data_staff_id);
                        $updated_allotted_slots = $row['allotted_slots'] + count($data_staff_id);
        
                        // Update the row in the database
                        $data = array(
                            'slots' => $updated_slots,
                            'pending_slots' => $updated_pending_slots,
                            'allotted_slots' => $updated_allotted_slots
                        );
        
                        $this->db->where('id', $insert_id);
                        $update_result = $this->db->update('hr_jp_interview_training', $data);
                    }
    ...
    public function get_allotted_slots($training_id) {
        // Adjust the table and column names based on your database structure
        $this->db->select('pendding_slots');
        $this->db->where('training_process_id', $training_id);
        $query = $this->db->get('hr_jp_interview_training');

        if ($query->num_rows() > 0) {
            $row = $query->row_array();
            return $row['pendding_slots'];
        } else {
            return 0; // Return 0 if the record is not found
        }
    }
    
    public function update_allotted_slots_join($training_id, $staffUserId, $training_name) {
    $this->db->set('staff_id', "CONCAT(IFNULL(staff_id,''), ',{$staffUserId}')", FALSE);
    $this->db->set('wait_staff_id', "TRIM(BOTH ',' FROM REPLACE(CONCAT(',', wait_staff_id, ','), ',{$staffUserId},', ','))", FALSE);
    $this->db->where('training_process_id', $training_id);
    $this->db->update('tblhr_jp_interview_training');
    $notified = add_notification([
        'description' => "You have been added in $training_name",
        'touserid' => $staffUserId,
        'link' => 'hr_profile/training?group=training_program',
        'additional_data' => serialize([
            "Contact With HR.",
        ]),
    ]);

    // Trigger notification using Pusher
    if ($notified) {
        pusher_trigger_notification([$staff_id]);
    }
    }
    
    public function update_wait_staff_id_cancel($training_id, $staffUserId, $training_name) {
    // Fetch the existing wait_staff_id value from the database
    $this->db->select('wait_staff_id');
    $this->db->where('training_process_id', $training_id);
    $existingWaitStaffId = $this->db->get('tblhr_jp_interview_training')->row()->wait_staff_id;

    // Remove the specified staffUserId from the existing wait_staff_id value
    $newWaitStaffId = implode(',', array_diff(explode(',', $existingWaitStaffId), [$staffUserId]));

    // Update the database with the modified wait_staff_id value
    $this->db->where('training_process_id', $training_id);
    $this->db->update('tblhr_jp_interview_training', ['wait_staff_id' => $newWaitStaffId]);

    $notified = add_notification([
        'description' => "You are not selected for $training_name",
        'touserid' => $staffUserId,
        'link' => 'hr_profile/training?group=training_program',
        'additional_data' => serialize([
            "Contact With HR.",
    ...
    public function update_allotted_slots($training_id, $staffUserId) {
        // print_r($training_id);die();
    // Increase allotted_slots by 1 and update staff_id
    $this->db->set('allotted_slots', 'allotted_slots+1', FALSE);
    $this->db->set('pendding_slots', 'pendding_slots-1', FALSE);
    $this->db->set('staff_id', "CONCAT(IFNULL(staff_id,''), ',{$staffUserId}')", FALSE);
    $this->db->where('training_process_id', $training_id);
    $this->db->update('tblhr_jp_interview_training');

    // Decrease pending_slots by 1
    
    // $this->db->where('training_process_id', $training_id);
    // $this->db->update('tblhr_jp_interview_training');
    }
    public function update_waitlist_slots($training_id, $staffUserId) {
    $this->db->set('wait_staff_id', "CONCAT(IFNULL(wait_staff_id, ''), IF(wait_staff_id IS NULL, '', ','), '{$staffUserId}')", FALSE);
    $this->db->where('training_process_id', $training_id);
    $this->db->update('tblhr_jp_interview_training');
    }
...
public function add_attachment_to_database($rel_id, $rel_type, $attachment, $external = false)
{
   ...
   public function get_staff_in_deparment($department_id)
   {
	...
	public function cancel_job_position_training_process($trainingid){
		//delete general info
		// Fetch the training name and staff ID before updating
        $this->db->select('training_name, staff_id');
        $this->db->where('training_process_id', $trainingid);
        $query = $this->db->get(db_prefix().'hr_jp_interview_training');
        ...
        if ($query->num_rows() > 0) {
            ...
            // Add notification
            $notified = add_notification([
                'description' => "Your training has been canceled",
                'touserid' => $staff_id,
                'link' => 'hr_profile/training?group=training_program',
                'additional_data' => serialize([
                    "Contact With HR.",
...
return false;
	...
	public function active_job_position_training_process($trainingid){
		//delete general info
		$data = array('status' => '0');
        $this->db->where('training_process_id', $trainingid);
        $this->db->update(db_prefix().'hr_jp_interview_training', $data);
    ...
    public function get_type_of_training($id = false){
        ...
        public function get_training_program($id = false){
    	if (is_numeric($id)) {
    		$this->db->where('training_process_id ', $id);
    	...
    	if ($id == false) {
    		return $this->db->query('select * from '.db_prefix().'hr_jp_interview_training order by training_process_id  desc')->result_array();
    	}

    }
    
            public function get_cost($id = false){
    	if (is_numeric($id)) {
    		$this->db->where('id ', $id);

    		return $this->db->get(db_prefix() . 'direct_cost_training')->row();
    	}

    	if ($id == false) {
    		return $this->db->query('select * from '.db_prefix().'direct_cost_training order by training_process_id  desc')->result_array();
    	}

    }
    
    public function getCostType($costid, $userId) {
    $this->db->select('costType');
    $this->db->where('costid', $costid); // Assuming 'costid' is the column in tblallocatecost representing costid
    $this->db->where('userId', $userId);
    $this->db->from(db_prefix().'allocatecost');

    $result = $this->db->get()->row_array();

    return ($result) ? $result['costType'] : null;
    }

    
    public function cost_data1() {
        $this->db->select('dt.*, tr.training_name');
        $this->db->from(db_prefix().'direct_cost_training dt');
        $this->db->join('tblhr_jp_interview_training tr', 'dt.training_program_id = tr.training_process_id', 'left');
        // $this->db->where_in('dt.id', $costids);
    ...
    public function get_components() {
        $this->db->select('*');
        $this->db->from(db_prefix().'components');
        // $this->db->where_in('dt.id', $costids);
    ...
    public function addelementscost($data)
    {
        // Insert data into the tblcomponents table
        $this->db->insert(db_prefix().'components', $data);

        // Return the inserted row ID
        return $this->db->insert_id();
    }
    
    public function cost_data($costids) {
        $this->db->select('dt.*, tr.training_name');
        $this->db->from(db_prefix().'direct_cost_training dt');
        $this->db->join('tblhr_jp_interview_training tr', 'dt.training_program_id = tr.training_process_id', 'left');
        $this->db->where_in('dt.id', $costids);
    ...
    public function faculty_data(){
    		...
    		return $this->db->query('select * from '.db_prefix().'faculty order by id  desc')->result_array();
    

    }
    
    public function faculty_data_tbltraining_attendance($wait_staff_id, $training_process_id){
    $query = 'SELECT * FROM tbltraining_attendance WHERE staff_id = ? AND training_id = ? ORDER BY id DESC';
    return $this->db->query($query, array($wait_staff_id, $training_process_id))->result_array();
    }

    
    public function faculty_data_id($id) {
    return $this->db->where('id', $id)->order_by('id', 'desc')->get(db_prefix() . 'faculty')->result_array();
    }
