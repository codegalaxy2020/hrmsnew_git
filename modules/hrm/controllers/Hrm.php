...
public function delete_hrm_staff_attachment($attachment_id)
    ...
    public function delete_insurance($insurance_id) {
        // Check if the user has permission to delete insurance
        if (!has_permission('hrm', '', 'delete')) {
            access_denied('hrm');
        }
    
        // Delete insurance record using Query Builder
        $this->db->where('insurance_id', $insurance_id);
        $result = $this->db->delete('tblstaff_insurance');
        ...
        if ($result) {
            ...
            echo json_encode(array('success' => false, 'message' => 'Failed to delete insurance item'));
        }
    }
